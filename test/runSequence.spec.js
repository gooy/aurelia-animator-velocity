...
describe('animator-velocity', () => {
  ...
  describe('runSequence function', () => {
    ...
    it('returns a promise', () => {
      let result = animator.runSequence(testSequence).catch((error) => console.log(error));
    ...
    it('publishes a sequenceBegin and sequenceDone event', (done) => {
      let sequenceBeginCalled = false, sequenceDoneCalled = false;
      let l1 = document.addEventListener(animationEvent.sequenceBegin, (payload) => sequenceBeginCalled = true),
        l2 = document.addEventListener(animationEvent.sequenceDone, () => sequenceDoneCalled = true);
    ...
    it('rejects the promise when nothing is passed', (done) => {
      animator.runSequence().catch((e)=>{ expect(e instanceof Error).toBe(true); done(); });
    });
    ...
    it('rejects the promise when an invalid array is passed', (done) => {
      animator.runSequence([1,2]).catch((e)=>{ expect(e instanceof Error).toBe(true); done(); });
    });

    it('rejects the promise when an HTMLElement is passed', (done) => {
      animator.runSequence(seq).catch((e)=>{ expect(e instanceof Error).toBe(true); done(); });
    });

    it('rejects the promise when a boolean is passed', (done) => {
      animator.runSequence(true).catch((e)=>{ expect(e instanceof Error).toBe(true); done(); });
    });
